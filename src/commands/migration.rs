...
pub fn execute_migration(connection: &Connection, migrations: Migrations) -> Result<()> {
    ...
    connection.execute_batch(migration_table_sql).or_else(|error| Err(Error::DatabaseError(error.to_string())))?;

    let mut statement = connection.prepare(&format!("SELECT name, hash FROM migrations_history ORDER BY id"))
        ...
        match migration_option {
            Some(migration) => {
                ...
                if name_value.as_str().unwrap() != name || hash_value.as_str().unwrap() != hash {
                    return Err(Error::MigrationError(format!("The migration {} has been modified", name)));
                }
            },
            None => {
                return Err(Error::MigrationError("The migration list has been modified".to_string()));
            }
        }
    }
    ...
    while let Some(migration) = migrations_iterator.next() {
        ...
        connection.execute_batch(sql)
            .or_else(|error| Err(Error::MigrationError(format!("Error executing migration: {}. {}", name, error.to_string()))))?;

        let mut statement = connection.prepare("INSERT INTO migrations_history (name, hash) VALUES (:name, :hash)")
            .or_else(|error| Err(Error::MigrationError(format!("Error preparing migration: {}. {}", name, error.to_string()))))?;

        statement.execute(&[(":name", name), (":hash", &&hash)])
...
mod tests {
    ...
    fn execute_migration_test() {
        ...
        let mut map = Map::new();
        map.insert("name".to_string(), JsonValue::String("create_test_table".to_string()));
        map.insert("sql".to_string(), JsonValue::String(create_table.to_string()));
        ...
        let mut map = Map::new();
        map.insert("name".to_string(), JsonValue::String("insert_test_data".to_string()));
        map.insert("sql".to_string(), JsonValue::String(insert_data.to_string()));
        migrations.push(JsonValue::Object(map));
        ...
        if let Some(row) = rows.next().unwrap() {
            ...
            assert_eq!(hash.as_str().unwrap(), calculate_hash(&create_table.to_string()));
        } else {
            assert!(false);
        }
        ...
        if let Some(row) = rows.next().unwrap() {
            ...
            assert_eq!(hash.as_str().unwrap(), calculate_hash(&insert_data.to_string()));
        } else {
            assert!(false);
        }
